...
@section('title', 'Tambah Pasien - Klinik Mediva')
    ...
    <div class="col-lg-10">
        <div class="card custom-card">
            <div class="card-header">
                <i class="bi bi-person-plus-fill text-success me-2"></i> Tambah Data Pasien Baru
            </div>
            <div class="card-body">
                <form action="{{ route('bidan.patients.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" name="gravida" class="form-control" value="{{ old('gravida') }}" min="0">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-semibold">Para (P)</label>
                            <input type="number" name="para" class="form-control" value="{{ old('para') }}" min="0">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-semibold">Abortus (A)</label>
                            <input type="number" name="abortus" class="form-control" value="{{ old('abortus') }}" min="0">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">HPHT</label>
                            <input type="date" name="hpht" class="form-control" value="{{ old('hpht') }}" id="hpht-input">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Taksiran Persalinan</label>
                            <input type="date" name="taksiran_persalinan" class="form-control" value="{{ old('taksiran_persalinan') }}" id="tp-input">
                        </div>
                    </div>

                    <div class="d-flex justify-content-between mt-4">
                        <a href="{{ route('bidan.patients.index') }}" class="btn btn-secondary"><i class="bi bi-arrow-left me-1"></i> Batal</a>
                        <button type="submit" class="btn btn-success"><i class="bi bi-check-lg me-1"></i> Simpan Data</button>
                    </div>
                </form>
            </div>
        </div>
